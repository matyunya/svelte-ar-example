...
ARjs.MarkerControls = THREEx.ArMarkerControls = function(
  ...
  function setParameters(parameters) {
    ...
    for (var key in parameters) {
      ...
      if (newValue === undefined) {
        console.warn(
          "THREEx.ArMarkerControls: '" + key + "' parameter is undefined."
        );
        continue;
      }
  ...
  if (_this.context.parameters.trackingBackend === "artoolkit") {
    ...
    // put aruco second
    this._arucoPosit = new POS.Posit(
      this.parameters.size,
      _this.context.arucoContext.canvas.width
    );
...
ARjs.MarkerControls.prototype._initArtoolkit = function() {
  ...
  function onMarkerFound(event) {
    ...
    if (
      event.data.type === artoolkit.BARCODE_MARKER &&
      event.data.marker.cfMatt < _this.parameters.minConfidence
    )
      return;

    var modelViewMatrix = new THREE.Matrix4().fromArray(event.data.matrix);

    console.log(modelViewMatrix, "ASDASD");
